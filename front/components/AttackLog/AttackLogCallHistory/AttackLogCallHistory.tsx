...
export const AttackLogCallHistory = () => {
	...
	const filteredData = mergedData.filter(
		(company: Company) => company.id?.toString() === companyId,
	);
					...
					<tbody className="border-solid border-2">
						{filteredData.map((company: Company, index: number) =>
							company.attackLogs.map((log: AttackLog, logIndex: number) => (
								<tr
								// 同じ会社内で同じ時刻に複数のログがない限り、各行に対して一意のキーを提供します。
								// logIndexも追加していますが、これは同じ時刻に複数のログが存在する可能性がある場合のため。
								key={`${company.id}-${log.calling_start}-${logIndex}`}
									className={logIndex % 2 === 0 ? "bg-white" : "bg-gray-100"}
								>
									<td className="border-2 text-nowrap truncate">
										{formatDateTime(log.calling_start)}
									</td>
